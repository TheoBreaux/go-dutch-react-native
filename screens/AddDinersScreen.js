...
const AddDinersScreen = () => {
  ...
  const checkIfDinerExistsInDatabase = async (username) => {
    ...
    try {
      const response = await fetch(
        `https://d580-2603-8000-c0f0-a570-a14e-4f47-d368-7088.ngrok-free.app/users/${username}`
      );
  ...
  useEffect(() => {
    const autoCompleteDiner = async () => {
      try {
        const response = await fetch(
          `https://d580-2603-8000-c0f0-a570-a14e-4f47-d368-7088.ngrok-free.app/additionaldiners/suggestions?input=${inputValue}`
        );
        const data = await response.json();
        setSuggestions(
    ...
    if (isValuePresent || !dinerInDatabase) {
      ...
      dispatch(
        addDiner({
          ...
          primaryPaymentSource: foundSuggestion.primaryPaymentSource,
          primaryPaymentSourceUsername:
            foundSuggestion.primaryPaymentSourceUsername,
          secondaryPaymentSource: foundSuggestion.secondaryPaymentSource,
          secondaryPaymentSourceUsername:
            foundSuggestion.secondaryPaymentSourceUsername,
        })
      );
  ...
  const postData = async () => {
    ...
    try {
      const response = await fetch(
        `https://d580-2603-8000-c0f0-a570-a14e-4f47-d368-7088.ngrok-free.app/additionaldiners`,
        {
          method: "POST",
